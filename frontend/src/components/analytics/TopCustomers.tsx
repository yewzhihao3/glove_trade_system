...
const TopCustomers: React.FC<TopCustomersProps> = ({ dateRange, onLoadChange }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await tradeService.getTopBuyers(8, dates);
  ...
  if (loading && data.length === 0) {
    return (
      <ChartCard title="Top Customers" description="Highest volume by company">
        <div className="w-full h-[300px] flex items-center justify-center">
            <div className="flex flex-col w-full px-8 justify-center gap-4 h-full animate-pulse opacity-50">
                {[80, 60, 90, 40, 70].map((w, i) => (
                    <div key={i} className="h-6 bg-slate-200 dark:bg-slate-700 rounded-r-sm" style={{ width: `${w}%` }} />
                ))}
            </div>
        </div>
      </ChartCard>
        ...
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="vertical" margin={{ top: 20, right: 20, left: 50, bottom: 5 }}>
              ...
              <YAxis dataKey="company_name" type="category" stroke="#94a3b8" tick={{ fill: '#94a3b8', fontSize: 11 }} width={120} />
