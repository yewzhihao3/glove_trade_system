...
@app.post("/leads/generate", response_model=List[schemas.BuyerLead])
def generate_leads(request: schemas.GenerateLeadsRequest, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
@app.get("/hscodes/generate", response_model=List[schemas.HSCodeBase])
def generate_hscodes(product_type: str, country: str, current_user = Depends(get_current_user)):
    results = ai_service.get_ai_hscodes(product_type, country)
    for res in results:
        res['country'] = country
    return results
